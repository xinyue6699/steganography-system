window.onload= function(){
    ...
    // 音频设置
    audio.src = "../../static/audio/prediction-1.mp3"
    ...
    var bbff=document.getElementById("bbff")
    bbff.onclick=bofang
    // 播放开始与暂停以及相关的图标字体修改
    function bofang() {
        if (audio.paused) {
    ...
    const audio_list = ['prediction-1', 'prediction-2', 'prediction-3', 'prediction-4', 'prediction-5']
    ...
    for (let i = 0; i < recm_list.length; i++) {
        recm_list[i].addEventListener('click', function() {
            audio.src = "../../static/audio/" + audio_list[i] + ".mp3"
            image.src = "../../static/image/main/" + image_list[i] + ".jpg"
